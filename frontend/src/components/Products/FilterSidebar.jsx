...
const [searchParams, setSearchParams] = useSearchParams(); //e.g. ?category=Top%20Wear&gender=Men
  ...
  const updateURLParams = (newFilters) => {
    ...
    Object.keys(newFilters).forEach((key) => {
      if (Array.isArray(newFilters[key]) && newFilters[key].length > 0) {
        params.append(key, newFilters[key].join(",")); //"XS,S,M"
  ...
  const handlePriceChange = (e) => {
    ...
    const newFilters = { ...filters, maxPrice: newPrice, minPrice: 0 };
    setFilters(newFilters);
    updateURLParams(newFilters);
  };
  return (
    <div className="p-4">
      <h2 className="text-xl font-medium text">Filter</h2>
      {/* Category Filter */}
      <div className="mb-6">
        ...
        {categories.map((category) => (
          <div key={category} className="flex items-center mb-2">
            <input
              ...
              className="mr-2 h-4 w-4text-blue-500 focus:ring-blue-400 border-grayy-300"
            />
          ...
          {colors.map((color) => (
            <button
              ...
              className={`w-8 h-8 rounded-full border border-gray-300 cursor-pointer transition hover:scale-105 ${
                filters.color === color ? "ring-2 ring-black-500" : ""
              }`}
            ></button>
          ))}
        </div>
